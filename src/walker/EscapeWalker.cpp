...
EscapeWalker::EscapeWalker(int d, int numSteps, const UniformRNG &rng_in, hull_algorithm_t hull_algo, bool amnesia)
    : SpecWalker<int>(d, numSteps, rng_in, hull_algo, amnesia)
{
    newStep = Step<int>(d);
    undoStep = Step<int>(d);
    winding_angle = std::vector<int>(numSteps + 1, 0);
...
void EscapeWalker::updateStepsFrom(int start)
{
    ...
    for(int i=start; i<numSteps; ++i)
    {
        ...
        if(d==2 && i > 0)
            winding_angle[i+1] = m_steps[i-1].winding_angle(next) + winding_angle[i];

        m_steps[i] = next;
        m_points[i+1] = head;
        occupied.emplace(head, i+1);
    }
}
